...
PSO = Optimizers.metaheuristic_list["PSO"]

all_curves = []
for _ in range(TRIALS):
    _, curve = PSO(ITER, HyperParameters.Parameters["num_individual"], obj_func).start()
    all_curves.append(curve)

all_curves = np.array(all_curves)
np.save(data_dir / "pso_curves.npy", all_curves)

first80 = []
for curve in all_curves:
    tgt = curve[-1] + 0.15 * (curve[0] - curve[-1])
    first80.append(next(i for i, v in enumerate(curve) if v <= tgt))
np.save(data_dir / "pso_first_reached.npy", np.array(first80))

np.save(data_dir / "pso_final.npy", all_curves[:, -1])
print("PSO done →", data_dir)
